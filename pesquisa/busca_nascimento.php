<?php
    ...
    //Busca
            ...
            $nascimento_pesquisa      = $_POST['nascimento'];
            //$pesquisa           = "SELECT * FROM cadastro WHERE nascimento LIKE '%$nascimento_pesquisa%' ";
            $pesquisa           = "SELECT * FROM cadastro  WHERE DATE_FORMAT(nascimento,'%m-%d') = DATE_FORMAT('$nascimento_pesquisa', '%m-%d') ORDER BY nome ASC; ";
               
            $resultado_pesquisa = mysqli_query($conecta, $pesquisa);
            
        //if($row_pesquisa = mysqli_fetch_assoc($resultado_pesquisa)){
